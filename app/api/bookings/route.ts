...
const createBookingSchema = z.object({
  ...
  estimated_price_min: z.number().nonnegative().optional(),
  estimated_price_max: z.number().nonnegative().optional(),
  items: z
    .array(
      z.object({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { items, estimated_price_range, estimated_price_min, estimated_price_max, ...bookingData } = result.data;

    // Calculate estimated value from price range midpoint (or fallback to weight×rate)
    const estimatedValue = (estimated_price_min != null && estimated_price_max != null)
    ...
    const { data: booking, error: bookingError } = await supabase
      ...
      .insert({
        ...
        estimated_price_min: estimated_price_min ?? null,
        estimated_price_max: estimated_price_max ?? null,
